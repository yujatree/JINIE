...
998	format(A,I5,A)
        ...
        open(100, file='HSH_C.out', status='unknown')
...
!--------------------------------------------------------------------------------------------
   ...
   function shell_c(dist) result(shl)

        double precision, intent(in) :: dist
        integer :: shl

        if      (dist > 16.800d0) then ; shl = 0
        else if (dist > 14.100d0) then ; shl = 5
        else if (dist > 11.075d0) then ; shl = 4
        else if (dist >  8.175d0) then ; shl = 3
        else if (dist >  4.900d0) then ; shl = 2
        else                           ; shl = 1
        end if

   end function shell_c
